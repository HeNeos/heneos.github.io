...
void moveLiquid(Bottle &fromBottle, Bottle &toBottle){
    int liquidToMove = fromBottle.topLiquid;
    int pos = fromBottle.liquidLevel - 1;
    fromBottle.state[pos] = 0;
    fromBottle.liquidLevel--;
    if(pos - 1 >= 0) fromBottle.topLiquid = fromBottle.state[pos-1];
    else fromBottle.topLiquid = 0;

    pos = toBottle.liquidLevel - 1;
    toBottle.topLiquid = liquidToMove;
    toBottle.state[pos+1] = liquidToMove;
    toBottle.liquidLevel++;
}

State moveLiquid(State& initialState, int from, int to){
    State newState = initialState;
    moveLiquid(newState.state[from], newState.state[to]);
...
void bfsLiquidPouring(State initialState) {
    ...
    // cout << encodeState(initialState) << endl;
    while (!q.empty()) {
        ...
        for (int i = 0; i < NOfBottles; i++) {
            for (int j = 0; j < NOfBottles; j++) {
                if (i != j) {
                    if(!isValidToMove(current.state[i], current.state[j])) continue;
                    State newState = moveLiquid(current, i, j);
